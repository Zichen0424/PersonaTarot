...
参考截图 `MainPage.png`、`TarotPage.png` 仅用于构图参考，不作为网页背景或牌面使用。角色立绘来自《女神异闻录3 Reload》官方中文网站，仅用作非官方同人网站的装饰。无战斗、奖励或解锁系统。英文字体 Barlow Condensed 随构建提供，其许可见依赖包内的 OFL；中文采用系统字体。运行时不依赖外部字体网站。
...
参考 `sample.mp4` 的斜切切场、悬浮卡牌、图鉴翻转和蓝色水波节奏实现：

- 模式/页面切换：青蓝色斜切遮罩与大字切场；背景水波、时钟刻度、光束与碎片缓慢移动。
- 牌阵：旋转错峰入场、轻微悬浮、选牌发牌、揭晓翻转及光带；洗牌时显示短暂的时钟隧道。
- 图鉴：切牌以透视旋转进入，当前卡片有青色辉光，正逆位旋转保持可读牌义。
- 顶部动效按钮可随时暂停；支持系统 `prefers-reduced-motion`。装饰不会阻挡点击，动效关闭后所有交互照常完成。
- 人物 WebP 以本地资源提供，无第三方图片请求；素材出处与权属见 [public/assets/CREDITS.md](public/assets/CREDITS.md)。卡牌仍为原创 SVG。
- 参考视频和截图不包含在发布内容中。新增动效样式独立放在 `src/motion.css`。
